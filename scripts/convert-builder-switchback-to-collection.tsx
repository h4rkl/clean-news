...
const BLOCK_REGEX =
  /<BuilderBlock\s+name="Switchback"\s+options=\{\{([\s\S]*?)\}\}\s*>\s*<\/BuilderBlock>/gs;
...
async function processFile(filePath: string): Promise<boolean> {
  ...
  if (matches.length > 1) {
    console.warn(`Multiple Switchback blocks found in ${filePath}. Skipping.`);
    return false;
  }
  ...
  const eyebrow = opts.eyebrow?.Default || "";
  const headline = opts.headline?.Default || "";
  const bodyHtml = opts.body?.Default || "";
  const bodyMd = htmlToMarkdown(bodyHtml);
  const imageSrc = `/builder${opts.image?.src}` || "";
  const imageAlt = opts.image?.alt?.Default || "";
  ...
  const title =
    headline || bodyMd.substring(0, 50) || `Switchback for ${postName}`;
...
async function main() {
  try {
    ...
    for (const file of mdxFiles) {
      ...
      if (changed) {
        converted++;
        console.log(`Converted Switchback in ${file}`);
      }
    }
